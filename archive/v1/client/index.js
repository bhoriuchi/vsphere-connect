...
module.exports = function(env) {
	...
	var Client = function(args) {
		...
		_client.getTypes              = require('./getTypes')(env, c);
		_client.errorHandler          = require('./errorHandler')(env, c);
		_client.method                = require('./method')(env, c);
		_client.propertySpec          = require('./propertySpec')(env, c);
		_client.logIn                 = require('./logIn')(env, c);
		_client.logOut                = require('./logOut')(env, c);
		_client.time                  = require('./time')(env, c);
		_client.emitEvents            = require('./emitEvents')(env, c);
		_client.emitUpdates           = require('./emitUpdates')(env, c);
		_client.getServiceContent     = require('./getServiceContent')(env, c);
		_client.getServiceProperties  = require('./getServiceProperties')(env, c);
		_client.retrieve              = require('./retrieve')(env, c);
		_client.monitor               = require('./monitor')(env, c);
		_client.findByName            = require('./findByName')(env, c);
		_client.findAllByDnsName      = require('./findAllByDnsName')(env, c);
		_client.findVmByDnsName       = require('./findVmByDnsName')(env, c);
		_client.findParentType        = require('./findParentType')(env, c);
		_client.destroy               = require('./destroy')(env, c);
		_client.rename                = require('./rename')(env, c);
			...
			return new Promise(function(resolve, reject) {
				soap.createClient(_client.wsdl, _client.options.soap, function(err, client) {
					if (err) {
						...
						// parse the types from the wsdl
						_client.types  = _client.getTypes();
						...
						if (_client.status === STATUS.UNINITIALIZED) {
							...
							.then(function(sc) {
								...
								_client.schema            = env.schema.utils.getSchema(_client.apiVersion);
