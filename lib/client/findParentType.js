	...
	function getParent(type, id, parentType, root) {
		...
		.then(function(obj) {
			...
			if (obj.length > 0) {
				...
				obj  = _.first(obj);
				type = _.get(obj, 'parent.type') ? _.get(obj, 'parent.type'): _.get(obj, 'parentVApp.type') ;
				id   = _.get(obj, 'parent.id') ? _.get(obj, 'parent.id'): _.get(obj, 'parentVApp.id') ;
