...
function getToken (headers) {
  return _.get(_.get(headers, 'set-cookie[0]', '')
    .match(/"(.*)"/), '[1]', null)
}
...
export default function login (username, password) {
  const isHostAgent = _.get(this, 'serviceContent.about.apiType')
    === 'HostAgent'
  const token = _.isString(username) && !password
    ? username
    : null
  ...
  if (token) {
    if (isHostAgent) {
      return Promise.reject(new Error('token authentication is '
